...
def add_engineered_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Claim_Amount_Per_Day"] = df["Claim_Amount"] / df["Length_of_Stay_Days"].replace(0, 1)
    df["Claim_Amount_Per_Procedure"] = df["Claim_Amount"] / df["Number_of_Procedures"].replace(0, 1)
    ...
    df["High_Previous_Claims_Provider"] = (df["Number_of_Previous_Claims_Provider"] > df["Number_of_Previous_Claims_Provider"].median()).astype(int) if df["Number_of_Previous_Claims_Provider"].notna().any() else 0
